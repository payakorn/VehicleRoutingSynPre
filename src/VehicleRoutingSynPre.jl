module VehicleRoutingSynPre

using Revise, Gurobi, JuMP, JLD2, Random, Glob, Printf, Combinatorics, SMTPClient, Markdown, PrettyTables, DataFrames, CSV, Latexify, Crayons, Weave, Random, Statistics, StatsBase, YAML, HiGHS, Tables, Graphs, SimpleWeightedGraphs, GraphRecipes, Plots, GraphPlot, LayeredLayouts, Luxor, Karnak

# Write your package code here.
include("ParticleSwarm.jl")
include("Solution.jl")
include("instance.jl")
include("random_problem.jl")
include("optimal.jl")

export
    Particle,
    IndexSolution,
    load_data,
    generate_empty_particle,
    generate_particles,
    find_group_of_node,
    find_starttime,
    find_starttime2,
    initial_insert_service,
    find_service_request,
    find_SYN,
    find_PRE,
    find_vehicle_service,
    feasibility,
    compatibility,
    find_compat_vehicle_node,
    create_empty_slot,
    find_vehicle_by_service,
    example,
    insert_vehicle_to_service,
    check_assigned_node,
    find_remain_service,
    total_distance,
    tardiness,
    objective_value,
    insert_PRE,
    complete,
    find_location_by_node_service,
    generate_example,
    example2,
    swap,
    move,
    List,
    save_particle,
    location_simulation,
    check_PRE,
    check_SYN,
    local_search,
    loop_test,
    choose_vehicle_from_service,
    path_relinking,
    local_search_ver2,
    check_all_syn_pre,
    check_all_insert_node,
    in_SYN,
    print_sol,
    load_problem,
    union_route,
    find_node_starttime,
    initial_starttime,
    PSO,
    load_ins,
    Sol,
    all_node_serv,
    sortperm_l,
    generate_empty_sol,
    inserted_node,
    insert_node_service,
    find_before_after_serv,
    starttime,
    random_move,
    test_forloop,
    load_example1,
    load_example1_par,
    in_same_route,
    load_ins_text,
    sent_email,
    dist,
    Ins,
    find_min_objective,
    create_csv_2014,
    df_conclusion_table,
    df_pretty_table,
    report,
    sent_email_report,
    generate_ins,
    run_gen_ins,
    find_ins,
    natural,
    dir,
    save_ins_txt,
    find_opt
end
